...
public class MenuPanel extends VBox {
    ...
    public MenuPanel() {
        ...
        headerPane.setPadding(new javafx.geometry.Insets(20, 0, 20, 16));
        ...
        // 应用名称
        appName = new Text("也字工厂");
        appName.setFont(FontUtil.loadFont("也字工厂淡淡黑", 16));
    ...
    private void addMenuItems() {
        // 推荐
        MenuItem recommend = new MenuItem("🏠", "推荐");
        recommend.setSelected(true);
        recommend.setHoverable(true);

        // 精选
        MenuItem jingxuan = new MenuItem("🎯", "精选");
        jingxuan.setHoverable(true);

        // 播客
        MenuItem podcast = new MenuItem("🎙️", "播客");
        podcast.setHoverable(true);

        // 漫游
        MenuItem roam = new MenuItem("🌍", "漫游");
        roam.setHoverable(true);

        // 关注
        MenuItem follow = new MenuItem("👤", "关注");
        follow.setHoverable(true);

        // 分隔线
        getChildren().add(new Separator());

        // 我的
        MenuItem myMusic = new MenuItem("❤️", "我喜欢的音乐");
        myMusic.setHoverable(true);

        MenuItem recent = new MenuItem("⏱️", "最近播放");
        recent.setHoverable(true);

        MenuItem myPlaylist = new MenuItem("🎵", "我的歌单");
        myPlaylist.setHoverable(true);

        MenuItem more = new MenuItem("⋮", "更多");
        more.setHoverable(true);

        // 分隔线
        getChildren().add(new Separator());

        // 创建的歌单
        MenuItem created = new MenuItem("📋", "创建的歌单");
        created.setHoverable(true);

        // 添加所有菜单项
        getChildren().addAll(recommend, jingxuan, podcast, roam, follow,
                myMusic, recent, myPlaylist, more, created);
    }
